function ButtonName = MessageBox(Question, Title, Btn1, Btn2, Default, varargin)
...
set(QuestFig, 'color',BackgroundColor)
...
MsgTxtXOffset=IconXOffset+IconWidth+IconXOffset;
...
% Adjust anchor point for other text alignments
switch HorizontalAlignment
    case 'center'
        MsgTxtXOffset = MsgTxtXOffset + MsgTxtWidth / 2;
    case 'right'
        MsgTxtXOffset = MsgTxtXOffset + MsgTxtWidth;
end
...
switch NumButtons
    case 1
        BtnXOffset = FigPos(3) / 2 - BtnWidth / 2;
    case 2
        BtnXOffset = [FigPos(3) / 2 - DefOffset - BtnWidth
                      FigPos(3) / 2 + DefOffset];
    case 3
        BtnXOffset = [FigPos(3) / 2 - BtnWidth / 2 - DefOffset - BtnWidth
                      FigPos(3) / 2 - BtnWidth / 2
                      FigPos(3) / 2 + BtnWidth / 2 + DefOffset];
end
BtnXOffset = BtnXOffset + IconWidth - IconXOffset;
...
if ishghandle(QuestFig)
    ...
    if strcmp(version('-release'), '2022a') % compatibility issue
        c = matlab.ui.internal.dialog.DialogUtils.disableAllWindowsSafely(1);
    else
...
drawnow; % Update the view to remove the closed figure (g1031998)
    ...
    function doDelete(varargin) %#ok<VANUS>
    delete(QuestFig);
    end
end
