...
n_cells = cell(nr_patch*nc_patch, 1); % just for debugging
if use_parallel
    parfor mpatch=1:(nr_patch*nc_patch)
        ...
        fprintf('Patch (%2d, %2d) is done. %2d X %2d patches in total. With %2d neurons \n', r, c, nr_patch, nc_patch, length(tmp_center));
        
        n_cells{mpatch}  = length(tmp_center); % use this to debug when too many neurons are picked
    end
    if any(cell2mat(n_cells) > 20), keyboard;end % this is to debug 
else
    for mpatch=1:(nr_patch*nc_patch)
        % get the indices corresponding to the selected patch
        tmp_patch = patch_pos{mpatch};
