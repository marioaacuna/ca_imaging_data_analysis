...
neuron_bk = neuron_concat.copy();
ctr = neuron_concat.estCenter(); 
K = size(neuron_concat.A,2);   %neuron numbers 
ind_del = false(K, 1);   
Coor = neuron_concat.get_contours(); 
[img, col0, AA] = neuron_concat.overlapA([], 0.1);  % overlap neurons
color_all = zeros(K,3);
col = col0; 
for m=1:3
...
pb_finish = uicontrol(fig_merge, 'style', 'pushbutton', ...
    ...
    'fontsize', 16,     'callback', 'neuron_concat.delete(ind_del); close(gcf); '); 
...
pb_new = uicontrol(fig_merge, 'style', 'pushbutton', ...
