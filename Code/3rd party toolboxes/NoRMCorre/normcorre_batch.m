...
while mod(T,bin_width) == 1
    if T == 1
        error('normcorre_batch:OneFrame','Movie appears to have only one frame. Use the function normcorre instead')        
    end
...
if col_shift 
    LOGGER.trace(sprintf('Offset %1.1d pixels due to bidirectional scanning detected',col_shift))
    if strcmpi(options.shifts_method,'fft')
        options.shifts_method = 'cubic';
        LOGGER.trace(sprintf('Cubic shifts will be applied')) 
    end
end
...
if nargin < 3 || isempty(template)
    LOGGER.trace(sprintf('Registering the first %i frames just to obtain a good template ...',init_batch))
...
%         M_final.Yr(d1*d2*d3,T) = zeros(1,data_type);        
    case {'hdf5','h5'}
         if exist(options.h5_filename,'file')
            ...
            LOGGER.trace(warning_msg)
            warning('normcorre_batch:FileExists', warning_msg)
    ...
    case {'tif','tiff'}
        ...
        opts_tiff.big = true;
        if nd == 3
            msg = 'Saving volumetric tiff stacks is currently not supported. Use a different filetype';
            LOGGER.critical(msg)
            error('normcorre_batch:VolumetricTiffStack',msg)
        end
    otherwise
        msg = 'This filetype is currently not supported';
        LOGGER.critical(msg)
        error('normcorre_batch:FileType',msg)
end   

cnt_buf = 0;
LOGGER.trace(sprintf('Template initialization complete.  Now registering all the frames with new template'))
%%
...
%                     M_final.Yr(:,t+lY-rem_mem:t+lY-1) = reshape(mem_buffer(1:d1*d2*d3*rem_mem),d1*d2*d3,rem_mem);
        ...
        LOGGER.trace(str, 'overwrite_last_message',~(it==1 & t==1))  % Overwrite message from the second iteration on
