...
if strcmpi(options.shifts_method,'fft')
    ...
    [xx_s, ~ ,yy_s, ~, zz_s, ~ ,xx_us, xx_uf, yy_us, yy_uf, zz_us, zz_uf] = ...
...
switch lower(options.output_type)
    ...
    case {'hdf5','h5'}
        if exist(options.h5_filename,'file')
            ...
            warning('NoRMCorre:hdf5_exists', '%s',warning_msg);
    ...
    case {'tif','tiff'}
        ...
        opts_tiff.big = true;
        if nd == 3
            error('NoRMCorre:Save3DTiff', 'Saving volumetric tiff stacks is currently not supported. Use a different filetype');
        end        
    otherwise
        error('NoRMCorre:FileTypeNotSupported', 'This filetype is currently not supported')
end 
...
if col_shift
    if strcmpi(options.shifts_method,'fft')
        ...
        LOGGER.trace(['Offset ', num2str(col_shift, '%1.1f'), ' pixels due to bidirectional scanning detected. Cubic shifts will be applied']); 
    end
end
...
%    if ~flag_constant
    ...
    LOGGER.trace([num2str(t+lY-1), ' out of ', num2str(T), ' frames registered'], 'overwrite_last_message', t>0)
end
