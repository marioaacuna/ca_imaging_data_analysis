function PROJECTIONS = make_trial_projections(animal_ID, filename_projections, experiment_name, varargin)
...
%         METADATA = SQL_database.read_table_where('sessions', {'+','animal_ID'}, animal_ID, 'animal_ID', 'return_all_sessions',true);
        ...
        if startsWith(animal_ID, 'LJ') && ~strcmp(experiment_name, 'Aversion_puff') % exception created for LJ data (fix later)
            group_sessions_by = {'date', 'stimulus'};
        else
...
%                     file_in = matfile(['L:\motion_correction_data_2p\', animal_ID,'.mat'], 'Writable',false);
    ...
    if print_figures
        ...
        for p = 1:n_projection_type
            ...
            fig = figure('color','k', 'units','normalized','outerposition',[0 0 1 1], 'Visible','off');
            imagesc(data2show);
            % Adjust axes
            colormap(gray)
            axis square
