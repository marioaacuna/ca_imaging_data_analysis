function single_cell_response_detection_AUC(all_animal_IDs, do_overwrite)
...
% List allowed compounds for the analysis
allowed_compounds = GC.analysis_allowed_compounds.(GC.experiment_name);

animal_names = SQL_database.read_table_where('experiments', {'animal_ID', 'experimental_group', 'n_ROIs'}, GC.experiment_name,'experiment_name');

% Loop through animals
for iid = 1:height(animal_names)
    % Skip if not completely analyzed
    animal_ID = animal_names.animal_ID{iid};

    % Skip if output file exists and user requested not to overwrite previous
    % runs of this script
    % Get name of folder where figures will be saved
    folder = [GC.data_root_path, GC.plots_path, GC.experiment_name, filesep, 'response_detection', filesep];
    if ~exist(folder, 'dir'), mkdir(folder), end
    figure_filename = [folder, animal_ID, '.pdf'];
    ...
    % Get statistics
    decoding_filename = get_filename_of('response_detection', animal_ID);
    results = load_variable(decoding_filename, 'RESULTS');

    % Read metadata for this animal
    METADATA = SQL_database.read_table_where('trials', {'date', 'stimulus', 'experimental_condition', 'frame_rate', 'Calcium_indicator', 'n_frames'}, animal_ID,'animal_ID');
    ...
    sessions = natsort(fieldnames(results));
    n_sessions = length(sessions);
    % Get number of cells
    stimuli = fieldnames(results.(sessions{1}));
    ...
    for istim = 1:length(stimuli)
        n_frames = METADATA{ismember(table2cell(METADATA(:, 'stimulus')), stimuli{istim}), 'n_frames'}; n_frames = n_frames(1);
        STIMULI.(make_variable_name(stimuli{istim}, false)) = Metadata_workflow.load_stimuli(stimuli{istim}, 'frame_rate',frame_rate, 'n_frames',n_frames);
    end
