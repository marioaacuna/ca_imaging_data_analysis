function response_detection_plot (all_animal_IDs, do_overwrite,experiment_name)
%% notes: if experiment_name = CLA_pain, delete the last trial, line6 -> check
disp('Plotting response detection')
global GC
...
%     GC.experiment_name = experiment_name;
    ...
    % Convert results
    results_filename = get_filename_of('response_detection_p_0_01', animal_ID, experiment_name);
    ...
    switch data_type{i_mouse}
        ...
        case 'epi'
            ...
            all_stim_metadata = stimuli_this_dataset;
...
%             GC.evoked_activity_max_latency = 10;
            evoked_window_n_bins = round(((GC.evoked_activity_max_latency * frame_rate)+1) / time_bin_activity_n_frames);
    end
        ...
        for i_stim =1:n_stimuli
            ...
            switch data_type{i_mouse}
                ...
                case 'epi'
                    ...
                    bins_to_cumulate(last_bin : end) = NaN;
            end
