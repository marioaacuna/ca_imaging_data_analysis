function dataset_response_modulation_AUROC_alluvial(all_animal_IDs, do_overwrite, do_summary, experiment_type)
...
GC.experiment_name = experiment_type ;
switch experiment_type
    case 'CLA_pain'
        STATS_response_modulation = load_variable(os.path.join(GC.repository_root_path, 'Figures_paper_CLA', '_data', 'SELECTIVITY_stats__p.mat'), 'SELECTIVITY_stats');
    case 'ACC_CCI_pain'
    ...
    case 'ACC_SNI_anxiety'
        STATS_response_modulation = load_variable(os.path.join(GC.repository_root_path, 'Figures_paper_SNI_anxiety', '_data', 'SELECTIVITY_stats__p.mat'), 'SELECTIVITY_stats');   
end
...
for iid = 1:length(all_animal_IDs)
    ...
    output_folder =  os.path.join(GC.data_root_path, GC.plots_path, GC.experiment_name, 'response_modulation');
    ...
    % runs of this script
    output_filename = os.path.join(output_folder, [animal_ID , '_class_identity.pdf']);
...
%     STATS = STATS(~ismember(fieldnames(STATS), 'SP'), :);
    ...
    try
        ...
        disp('Double checking of ROIs does not exist')
%         continue
    end
    keep_cells = [1:size(STATS,1)]';
    ...
    switch experiment_type
        case 'ACC_CCI_pain'
            ...
            sessions_to_plot = 3:5;
            group_colors = {'HPS',[1,0,0]; 'puff',[.05,.48,.75]; 'mixed',[1,.65,0]; 'none',[.7,.7,.7]};
        case 'ACC_SNI_anxiety'
            stim_to_plot = stimuli(ismember(stimuli, {'heat', 'cold'}));
            sessions_to_plot = 1 : size(STATS.(stim_to_plot{1}),2);
            group_colors = {stim_to_plot{1},[1,0,0]; stim_to_plot{2},[.05,.48,.75]; 'mixed',[1,.65,0]; 'none',[.7,.7,.7]};
        otherwise
            keyboard % determine stimuli and sessions to plot
    end
  
 
    n_sessions_to_plot = 3;%length(sessions_to_plot);
    METADATA = SQL_database.read_table_where('sessions', {'+'}, animal_ID, 'animal_ID');
%         STATS = STATS_response_modulation.(animal_ID);
%         STATS.day_from_surgery = cellfun(@str2double, STATS.day_from_surgery);
    sessions = unique(METADATA.day_from_surgery, 'stable');
...
%     n_selective = length(unique(STATS.ROI_id));
    n_selective = NaN(n_all_cells, length(stim_to_plot));
    for i_stim = 1:length(stim_to_plot) 
        this_stim = stim_to_plot{i_stim};
        n_selective(:, i_stim) = sum(~isnan(STATS.(this_stim)(:,sessions_to_plot)),2);
    end
     n_not_selective = length(find(sum(n_selective, 2) == 0));
     n_selective = n_all_cells - n_not_selective;
...
%                     AUROC = stats_of_this_timepoint{ismember(stats_of_this_timepoint.stimulus, selectivities{istim}), 'AUROC'};
                    ...
                    preferred_stimulus = [preferred_stimulus, stim_to_plot{istim}, preference_sign];
%                 end
            end

            % Store selectivity in main variable
%             day_index = ismember(sessions, timepoints(itimepoint));
            day_index = itimepoint;
%             if ~strcmp(preferred_stimulus, '')
...
%    data(ismember(DATA, 'none')) = Inf;
    data(sum(data,2) == 0,:) = Inf;

    % Mark missing data with -1
    data(ismember(DATA, '') & ~ismember(data, Inf)) = -1;
    DATA(ismember(data, -1))= {'none'};
...
% 
        data(data == 0) = length(stim_to_plot) + 1;
    else
        for imix = 1:length(mixed_selectivities)
            data(ismember(DATA, mixed_selectivities{imix})) = length(stim_to_plot) + 1 + imix;
...
%     bad_rows = any(data' == -1);
    ...
    PARAMETERS.time_step = 1:3;%find(sessions > 0, 1, 'first'); %3
