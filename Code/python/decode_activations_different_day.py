...
def run(folder, output_filename_results, output_filename_confusion_matrix, verbose=False):
    ...
    if n_classifiers == 0:
        ...
        exit(0)
    results_to_load = sorted([i for i in files if i.endswith('_decoding_results.json')])
    if verbose:
    ...
    if n_classifiers >= 2:
        ...
        for i_pair in range(n_pairs):
            ...
            AUPRC, cm = clf_day2.compute_AUC(labels_day2, posterior_probabilities_day2_with_clf_day1, metric='PR', return_average=False, return_confusion_matrix=True, normalize_AUPRC_to_random=True)
            AUPRC_day2_with_clf_day1 = {key: value for key, value in zip(tested_classes_day2, AUPRC)}
            AUPRC_day2_with_clf_day2 = results[pairs[i_pair][1]]['performance']['AUPRC']
            ...
            # Store data
            [PERFORMANCE[key].append([str(pairs[i_pair][0] + 1), str(pairs[i_pair][1] + 1), AUPRC_day2_with_clf_day2[key], str(AUPRC_day2_with_clf_day1[key])]) for key in tested_classes_day2]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        folder = r'V:\Ca_imaging_pain\6_data\ACC_CCI_anesth\\response_decoding\\FK_10\\'
        run(folder=folder, output_filename_results='', output_filename_confusion_matrix='', verbose=True)
