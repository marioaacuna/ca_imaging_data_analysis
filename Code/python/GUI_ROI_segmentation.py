...
with warnings.catch_warnings():
    ...
    try:
        ...
        sobel = flood = convex_hull_image = None
        HAS_SKIMAGE = False
...
class Calcium_imaging_data_explorer(object):
    def __init__(self, PARAMETERS, app):
        ...
        if os.path.splitext(self.PARAMETERS['filename_data_time'])[1] == '.npy':
            ...
            self.data_time = tb.File(self.PARAMETERS['filename_data_time'], mode='r').get_node(where='/', name='time_first')
        self.time = np.linspace(0, self.PARAMETERS['n_frames'] / self.PARAMETERS['frame_rate'], self.PARAMETERS['n_frames'])
        self.time_range = [self.time.min(), self.time.max()]
        self.session_names = self.PARAMETERS['session_order']
    ...
    def initialize_GUI(self):
        ...
        average_types = list()
        # Set length of variable for averages
        self.data_average = dict()
        # Get length of data_average variable
        n_frames = list()
        with h5py.File(self.PARAMETERS['filename_projections'], 'r', libver='latest') as file_in:
            projections = file_in['PROJECTIONS']
            if np.size(self.session_names) == 1:
                session_name = self.session_names
                average_types = list(projections[session_name].keys())
                shape = list(projections[session_name][average_types[0]].shape)
                if len(shape) == 2:
                    shape = [1] + shape
                n_frames_this_session = shape[0]
                n_frames.append(n_frames_this_session)
            else:
        ...
        n_frames = np.cumsum(n_frames).reshape(-1, 1)
        n_frames = np.hstack((np.vstack(([0], n_frames[:-1])), n_frames))
        empty_average_variable = np.zeros((n_frames[-1, -1], self.PARAMETERS['frame_height'], self.PARAMETERS['frame_width']), dtype=np.float64)
        for avg_typ in average_types:
            self.data_average[avg_typ] = empty_average_variable.copy()
        # Get average frames
        with h5py.File(self.PARAMETERS['filename_projections'], 'r', libver='latest') as file_in:
            projections = file_in['PROJECTIONS']
            if np.size(self.session_names) == 1:
                ...
                session_idx = 0
                average_types = list(projections[session_name].keys())
                frame_start = n_frames[session_idx, 0]
                frame_end = n_frames[session_idx, 1]
                for avg_typ in average_types:
                    # Copy and transpose dimensions
                    projection_image = projections[session_name][avg_typ][:]
                    if projection_image.ndim == 2:
                        projection_image = projection_image.reshape(1,
                                                                    projection_image.shape[
                                                                        0],
                                                                    projection_image.shape[
                                                                        1])
                    self.data_average[avg_typ][frame_start:frame_end, :,
                    :] = projection_image.transpose((0, 2, 1))
            else:
                for session_idx, session_name in enumerate(self.session_names):
                    average_types = list(projections[session_name].keys())
                    frame_start = n_frames[session_idx, 0]
                    frame_end = n_frames[session_idx, 1]
                    for avg_typ in average_types:
                        ...
                        projection_image = projections[session_name][avg_typ][:]
                        if projection_image.ndim == 2:
                            projection_image = projection_image.reshape(1, projection_image.shape[0], projection_image.shape[1])
                        self.data_average[avg_typ][frame_start:frame_end, :, :] = projection_image.transpose((0, 2, 1))
        ...
        pixel_list = pixel_list.reshape(self.PARAMETERS['frame_width'], self.PARAMETERS['frame_height']).transpose().ravel()
        ...
        # Set image
        self.frame_image_view.setImage(self.data_frame, autoRange=True, axes={'t': 0, 'x': 2, 'y': 1}, autoLevels=True)
        ...
        # Add boundaries of each session
        for ii in range(self.PARAMETERS['sessions_last_frame'].shape[0] - 1):  # -1 because we ignore the last idx
            pos = self.PARAMETERS['sessions_last_frame'][ii] / self.PARAMETERS['frame_rate']
            l = pg.InfiniteLine(pos=pos, angle=90, pen=pg.mkPen(color=(77, 77, 77)))
            l.setZValue(1)
    ...
    def callback_save(self, compute_traces=False):
        ...
        TRANSFORMATION_MATRICES = np.atleast_2d(self.TRANSFORMATION_MATRICES.values)
        TRANSFORMATION_MATRICES = np.hstack((TRANSFORMATION_MATRICES, np.array(self.TRANSFORMATION_MATRICES.index).reshape(-1, 1)))
    ...
    def callback_load(self):
        ...
        if not os.path.exists(self.PARAMETERS['filename_output']):
            log('File \'%s\' does not exist' % self.PARAMETERS['filename_output'])
            return
        # Count ROIs
        n_ROIs = self.ROI_TABLE.shape[0]
        if n_ROIs != 0:
        ...
        load_rest_of_file = True
        # Unpack variables
        ROI_info = np.atleast_2d(file_content['ROI_info'])
        process_ROIs = ROI_info.size > 0
        ...
        if file_content['TRANSFORMATION_MATRICES'].shape[0] != 0 and load_rest_of_file:
            TRANSFORMATION_MATRICES = np.vstack(file_content['TRANSFORMATION_MATRICES'])
            if TRANSFORMATION_MATRICES.shape[1] > len(list(self.TRANSFORMATION_MATRICES.columns)):
                index = TRANSFORMATION_MATRICES[:, -1]
                TRANSFORMATION_MATRICES = TRANSFORMATION_MATRICES[:, :-1]
            else:
                index = np.arange(TRANSFORMATION_MATRICES.shape[0])
            self.TRANSFORMATION_MATRICES = pd.DataFrame(TRANSFORMATION_MATRICES, index=index, columns=self.TRANSFORMATION_MATRICES.columns)
        ...
        has_condition_idx = np.where(np.in1d(self.PARAMETERS['condition_names'], self.TRANSFORMATION_IDX[:, 2]))[0]
        add_rows_for_condition = not np.array_equal(all_condition_idx, has_condition_idx)
        if add_rows_for_condition:
            # Make empty arrays
        ...
        self.MAPPING_TABLE = np.tile(np.atleast_2d(pixel_list), (self.n_conditions, 1)).astype(np.float32)  # Float32 conversion to hold NaNs
        # Initialize lookup table of frames that have a modified mapping
        self.modified_MAPPING = np.zeros(shape=self.n_conditions, dtype=bool)

        # Check whether to load the info on mapping
        load_mapping_from_file = file_content['MAPPING_TABLE'].shape[0] != 0 and \
                                 file_content['modified_MAPPING'].shape[0] != 0 and \
                                 load_rest_of_file
        if load_mapping_from_file:
            # Copy pixel mapping information
            self.MAPPING_TABLE[has_condition_idx, :] = np.atleast_2d(file_content['MAPPING_TABLE'])
            self.modified_MAPPING[has_condition_idx] = np.array(file_content['modified_MAPPING']).astype(bool)
        ...
        if process_ROIs:
            ...
            # If there are ROIs present, enable the tool that allows to translate the field of view
            self.buttons.translation_ROI.setEnabled(process_ROIs)
    ...
    def make_region(self, regions):
        ...
        for ireg in range(regions.shape[0]):
            ...
            # Set region (in time)
            reg2.setRegion(regions[ireg, :] / self.PARAMETERS['frame_rate'])
            reg2.setZValue(0)
    ...
    def compute_frames_projection(self, return_image=False, force_update_histogram=False):
        ...
        n_frames = len(self.current_average_frame)
        for iframe in range(n_frames):
            T = self.TRANSFORMATION_IDX[self.TRANSFORMATION_IDX[:, 0].astype(int) == self.current_average_frame[iframe], 1][0]
            if len(T) > 0:
                # Apply transformations to current frame one after the other
                for t_idx in T:
                    # Get matrices
    ...
    def callback_draw_ROI(self):
        roi = last_row = shown_traces = None
        ...
        if self.current_action == 'drawing_ROI':
            ...
            # Compute points
            if HAS_SKIMAGE and self.selected_ROI_type == 'fill':
    ...
    def callback_update_zoom_ROI(self, roi, update_trace=True, update_ROI_contour=True):
        L = temp_modified_conditions_idx = None
    ...
    def make_ROI_from_point(self, xy_point):
        ...
        image_sobel = sobel(image, sobel_mask)
    ...
    def callback_accept_translation_FOV(self):
        if self.current_action == 'translating_FOV' and not self.translation_ROI.isMoving:
            ...
            if self.TRANSFORMATION_MATRICES.shape[0] == 0:
                table_idx = 0
            else:
                table_idx = np.max(np.array(self.TRANSFORMATION_MATRICES.index)) + 1
    ...
    def callback_timeline_frames(self, from_plot):
        ...
        if not self.updating_timeline_position:
            ...
            if from_plot == 'frame':
                ...
                time = time / self.PARAMETERS['frame_rate']
...
if __name__ == '__main__':
    ...
    if len(sys.argv) <= 1:
        ...
        params_filename =  r'D:\_MATLAB_CaImaging\NRN_02\NRN_02_params.mat'

    else:  # If called from command line
    ...
    if not os.path.exists(parameters['filename_data_frame']) or not os.path.exists(parameters['filename_data_time']) or do_only_conversion:
