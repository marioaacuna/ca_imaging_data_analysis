...
def cmapToColormap(cmap, nTicks=16):
    ...
    *nTicks*: Number of ticks to create when dictionary of functions is used. Otherwise unused.
    """
    ...
    if hasattr(cmap, '_segmentdata'):
        ...
        if ('red' in colordata) and isinstance(colordata['red'], collections.Sequence):
            ...
            # collect the color ranges from all channels into one dictionary to get unique indices
            posDict = {}
...
def initialize_field(obj, field_name, value_type="class", initial_value=None, shape=None):
    ...
    class subfield(dict):
        def __init__(self, name="subfield"):
            ...
            # dictionary.subfield works as dictionary["subfield"]
            self.__dict__ = self
...
def transform_image_from_parameters(image, scale, angle, offset, return_matrices=False):
    ...
    if np.any(scale != 1):
        ...
        # Apply scaling
        image = cv2.resize(image, None, fx=scale[0], fy=scale[1], interpolation=interpolation)
    ...
    frame_size_width = image.shape[1]
    frame_size_height = image.shape[0]
    canvas_size = np.array([frame_size_height, frame_size_width])
    if angle != 0:  # Add space for complete rotation (e.g., 45 degrees)
        canvas_size = canvas_size * np.sqrt(2)
    if np.any(offset != 0):
        canvas_size = canvas_size + (np.abs(offset).max() * 2)  # Add double the max offset to allow offsets in any direction
    # Make it an integer multiple of 2
    canvas_size = np.ceil(canvas_size)
    canvas_size = (np.floor(canvas_size / 2.) * 2.).astype(int)
...
def transform_image_from_matrices(image, scaling_matrix, rotation_matrix, translation_matrix):
    ...
    if len(scaling_matrix) == 3 and np.any(scaling_matrix[:2] != 1):
        ...
        image = cv2.resize(image, None, fx=scaling_matrix[1], fy=scaling_matrix[0], interpolation=int(scaling_matrix[2]))
    ...
    if not isinstance(rotation_matrix, np.ndarray) and rotation_matrix == 0:
        ...
        #         center_y = canvas_margin[0] + image.shape[0] / 2
        #         center_x = canvas_margin[1] + image.shape[1] / 2
        #         # Grab the rotation matrix (applying the negative of the angle to
        #         # rotate clockwise), then grab the sine and cosine (i.e., the rotation
        #         # components of the matrix)
        #         rotation_matrix = cv2.getRotationMatrix2D((center_y, center_x), -angle, 1.0)

        # Perform rotation of the image
        # image = cv2.warpAffine(canvas, rotation_matrix, None, flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=np.nan)
