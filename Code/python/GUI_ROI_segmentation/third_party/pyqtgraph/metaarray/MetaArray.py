...
class MetaArray(object):
    ...
    descriptions. An additional dictionary at the end of the axis list may specify parameters
    ...
    def listColumns(self, axis=None):
        """Return a list of column names for axis. If axis is not specified, then return a dictionary of {axisName: (column names), ...}."""
        if axis is None:
            ret = {}
    ...
    def writeHDF5Meta(self, root, name, data, **dsOpts):
        if isinstance(data, np.ndarray):
            ...
            gr = root.create_group(name)
            gr.attrs['_metaType_'] = 'dictionary'
