...
def objString(obj):
    ...
    try:
        if type(obj) in [int, float]:
            ...
            if len(obj) > 5:
                return "<dictionary {%s,...}>" % (",".join(list(obj.keys())[:5]))
            else:
                return "<dictionary {%s}>" % (",".join(list(obj.keys())))
...
def pretty(data, indent=''):
    """Format nested dictionary/list/tuple structures into a more human-readable string
