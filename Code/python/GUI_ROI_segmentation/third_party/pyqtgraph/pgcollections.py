...
class ReverseDict(dict):
    """extends dictionary so that reverse lookups are possible by requesting the key as a list of length 1:
       d = BiDict({'x': 1, 'y': 2})
       d['x']
         1
       d[[2]]
...
class BiDict(dict):
    """extends dictionary so that reverse lookups are possible by adding each reverse combination to the dictionary.
...
class ThreadsafeDict(dict):
    """Extends dictionary so that getitem, setitem, and contains are all thread-safe.
...
class CaselessDict(OrderedDict):
    """Case-insensitive dictionary. Values can be set and retrieved using keys of any case.
    ...
    def __init__(self, *args):
        ...
        if len(args) == 0:
            ...
            raise Exception("CaselessDict may only be instantiated with a single dictionary.")
...
class ProtectedDict(dict):
    """
    A class allowing read-only 'view' of a dictionary. 
    The object can be treated like a normal dictionary, but will never modify the original dictionary it points to.
    Any values accessed from the dictionary will also be read-only.
    """
...
class ProtectedList(collections.Sequence):
    """
    A class allowing read-only 'view' of a list or dictionary. 
