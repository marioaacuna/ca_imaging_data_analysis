...
def genString(data, indent=''):
    ...
    for k in data:
        ...
        if len(sk) == 0:
            print(data)
            raise Exception('blank dictionary keys not allowed (see data above)')
        if sk[0] == ' ' or ':' in sk:
            print(data)
            raise Exception('dictionary keys must not contain ":" or start with spaces [offending key is "%s"]' % sk)
