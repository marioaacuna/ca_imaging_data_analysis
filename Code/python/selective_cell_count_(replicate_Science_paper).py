...
stimuli_groups = [['nox. heat', ['temp_48']],
                  ['shock', ['HPS', 'FPS']],
                  ['aversive', ['puff', 'sound']],
                  ['heat 43', ['temp_43']],
                  ['heat 38', ['temp_38']],
...
cell_count = np.zeros((n_combinations, n_animals), dtype=np.float32) * np.nan
...
for iid, animal_ID in enumerate(animals):
    ...
    if sessions_before_surgery.shape[0] == 0:
        continue
    ...
    this_cell_count = this_cell_count[:, sessions_before_surgery]
    # Average across sessions
    this_cell_count = np.nanmean(this_cell_count, axis=1)

    # Add number of cells that were stimulated with these stimuli
    n_cells_per_stimuli_combination[np.logical_not(np.isnan(this_cell_count))] += n_cells

    # Store values
    cell_count[:, iid] = this_cell_count
...
# Set colors
color_all_cells = [.92, .89, .85]
color_noxious_heat = [.93, .25, .20]
...
# Nociceptive cells out of the total
total = data_to_plot['value'].sum()
nox_heat = data_to_plot.loc[np.where(data_to_plot['nox. heat'] == True)[0], 'value'].sum()
plt.figure(facecolor='w')
V = venn2(subsets=[total-nox_heat, 0, nox_heat], set_labels=['All cells', 'Nox. heat'], alpha=1.0, subset_label_formatter=lambda x: '%i%%' % np.round(x, decimals=0))
V.subset_labels[1].set_visible(False)  # It is 0
V.subset_labels[0].set_text('100%')
V.patches[0].set_facecolor(color_all_cells)
V.patches[2].set_facecolor(color_noxious_heat)


stimuli_to_take = ['nox. heat', 'heat 43', 'heat 38']
...
for ii, lab in enumerate(labels):
    ...
    subsets[lab] += value
plt.figure(facecolor='w')
V = venn3(subsets, set_labels=stimuli_to_take, set_colors=[color_noxious_heat, color_heat_43, color_heat_38], alpha=1, subset_label_formatter=lambda x: '%i%%' % np.round(x, decimals=0))
...
for ii, lab in enumerate(labels):
    ...
    subsets[lab] += value
plt.figure(facecolor='w')
V = venn3(subsets, set_labels=stimuli_to_take, set_colors=[color_nociceptive, color_shock, color_aversive], alpha=1, subset_label_formatter=lambda x: '%i%%' % np.round(x, decimals=0))
