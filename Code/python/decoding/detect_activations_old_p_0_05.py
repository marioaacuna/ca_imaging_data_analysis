...
def _compute_accuracy_observation_subset(data: np.ndarray, bins_to_cumulate: np.ndarray,
    ...
    if idx.shape[0] == 0:
        ...
        significant_trials_idx = [np.where(np.logical_and(P_values[:, i] <= 0.05, data_diff[:, i] != 0))[0].tolist() for i in range(bin_markers.shape[0])]
        performance_single_cell['significant_trials'] = significant_trials_idx
        # Find significant activations
        significant_activations_idx = np.where(np.logical_and(p_values <= 0.05, np.sign(AUC - np.mean(null_distribution, axis=0)) == idx[:, 3]))[0]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        run(input_filename=r"D:\_MATLAB_CaImaging\MA_29epi\session190821_heat_detection_data.mat", verbose=True)
